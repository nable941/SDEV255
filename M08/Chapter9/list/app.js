...
var listSchema = mongoose.Schema({text : String});
...
//determined correction with CHAT GPT
app.post("/list", async function(req, res) {
  try {
    var text = req.body.text;
    var doc = await List.create({ text: text });
    res.json({ id: doc._id }); // send the MongoDB identifier in the response
  } catch (err) {
    // handle error here, e.g., send a response with a 500 status code
